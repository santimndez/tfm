...
Based in the segmenter module of TT3D: https://github.com/cogsys-tuebingen/tt3d/blob/main/tt3d/traj_seg/segmenter.py
"""
...
def fit_polynomial(t, traj):
    ...
    :param t: Time points of the trajectory (N,).
    :param traj: Trajectory points (N, K).
    :return coeffs: Coefficients of the fitted polynomial (3, K).
    """
...
def find_intersection(coeffs1, coeffs2):
    ...
    :param coeffs1: Coefficients of the first polynomial (D, K).
    :param coeffs2: Coefficients of the second polynomial (D, K).
    :return: Intersection points (D, K).
    """
...
def get_accurate_bouncing_pose(t_before, traj_before, t_after, traj_after, t_margin=0.045):
    """
    ...
    :param t_before: Time points before the bounce.
    :param traj_before: Trajectory points (x, y) before the bounce.
    :param t_after: Time points after the bounce.
    :param traj_after: Trajectory points (x, y) after the bounce.
    :param t_margin: Time margin to ensure the bounce time is not too close to the segment boundaries.
    ...
    t_min, t_max = t_before.max() - t_margin, t_after.min() + t_margin
...
def fit_quad_and_sse(t_segment, y_segment):
    # t_segment: (m,), y_segment:(m,2)  (x,y)
    A = np.vstack([np.ones_like(t_segment), t_segment, t_segment**2]).T  # (m,3)
    # solve for both coordinates together (multioutput)
    coeffs, *_ = np.linalg.lstsq(A, y_segment, rcond=None)  # coeffs shape (3,2)
    pred = A.dot(coeffs)  # (m,2)
    sse = np.sum(np.abs(y_segment - pred)) # l1 loss
    return sse, coeffs


def dp_segmenter(t, traj, L=DEFAULT_LOSS):
    """
    Segments trajectory data based on quadratic polynomial fitting with penalties using basic dynamic programming.

    :param t: Array of time or independent variable (n,).
    :param traj: Trajectory data with shape (n, 2) for X and Y image coordinates.
    :param L: Penalty for each segment.
    ...
    # precompute SSE y coef para todos los intervalos i..j
    ...
    while j > 0:
        i = prev[j]
        breakpoints.append(i)  # segment [i, j)
        j = i
    breakpoints.reverse()

    return np.array(breakpoints)

def basic_segmenter(t, traj, deg=2, L=DEFAULT_LOSS, use_blur=False):
    """
    Segments trajectory data based on polynomial fitting with penalties.

    :param t: Array of time or independent variable (n,).
    :param traj: Trajectory data with shape (n, 2) for X and Y image coordinates.
    :param deg: Polynomial degree for each segment.
    :param L: Penalty for each segment.
